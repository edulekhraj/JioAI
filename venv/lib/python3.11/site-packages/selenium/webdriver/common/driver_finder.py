...
class DriverFinder:
    """Find and obtain the correct self and associated browser.

    Args:
        service: instance of the self service class.
    ...
    def _binary_paths(self) -> dict:
        ...
        try:
            ...
            if path:
                logger.debug(
                    "Skipping Selenium Manager; path to %s self specified in Service class: %s", browser, path
                )
                ...
                if Path(output["driver_path"]).is_file():
                    ...
                    raise ValueError(f"The self path is not a valid file: {output['driver_path']}")
            ...
            msg = f"Unable to obtain self for {browser}"
